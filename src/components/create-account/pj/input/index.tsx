...
interface Props {
  title: string;
  id: keyof BodyTypePj;
  register: UseFormRegister<BodyTypePj>;
  errors: FieldErrors<BodyTypePj>;
  registerName: keyof BodyTypePj;
  optional?: boolean;
  placeholder: string;
  type?: string;
  watch?: UseFormWatch<BodyTypePj>;
  children?: ReactNode;
  w?: string;
  setValue?: UseFormSetValue<BodyTypePj>;
  trigger?: UseFormTrigger<BodyTypePj>;
  setError?: UseFormSetError<BodyTypePj>;
  isLoading?: boolean;
...
export default function Input({
  ...
  const handleInputCnpj = (event: FormEvent<HTMLInputElement>) => {
    ...
    if (value.length > 6) {
      value = value.replace(/\D/g, '').replace(/^(\d{2})(\d{3})/, '$1.$2.');
    }
    if (value.length > 10) {
      value = value
        .replace(/\D/g, '')
    ...
    <div className={`flex gap-[5px] flex-col items-start ${w}`}>
      <div className="flex flex-col gap-1 items-start w-full">
        <Label
          ...
          optional={optional}
        />
        {(registerName === 'corporateName' || registerName === 'stateRegistration' || registerName === 'fullName' || registerName === 'email') && ( // eslint-disable-line
          <input
